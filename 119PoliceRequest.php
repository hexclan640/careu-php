...
$stmt = $conn->prepare(" SELECT request.requestId,request.date ,request.time , request.type , 119policerequest.complainCategory ,description FROM request INNER JOIN 119policerequest ON request.requestId = 119policerequest.requestId WHERE request.userId = '$userId';");
 $stmt->execute();
$product = array();

$stmt->bind_result($requestId,$date,$time,$type,$category,$description);
